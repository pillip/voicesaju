...
export interface MeEditSajuPageProps {
  /**
...
export default function MeEditSajuPage({ fetchImpl }: MeEditSajuPageProps) {
  ...
  if (state.kind === "quota_exhausted") {
    return (
      <div className="flex min-h-screen flex-col bg-ink-900 text-cream-100">
        ...
        >
          <p className="font-body text-base text-cream-200">
            수정 한도(2회)를 모두 사용했어요.
          </p>
        ...
        >
          ...
          <label className="flex flex-col gap-s1 font-body text-sm">
            ...
            <input
              ...
              onChange={(e) =>
                setForm((p) => ({ ...p, birthTime: e.target.value }))
              }
          ...
          <label className="flex items-center gap-s2 font-body text-sm">
            <input
              ...
              onChange={(e) =>
                setForm((p) => ({ ...p, isLunar: e.target.checked }))
              }
