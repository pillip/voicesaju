...
import { MeEditSajuView as MeEditSajuPage } from '@/app/me/edit-saju/MeEditSajuView';
...
describe('MeEditSajuPage', () => {
  ...
  it('renders the form + counter banner after a successful profile load', async () => {
    const fetchImpl = vi.fn().mockResolvedValueOnce(mkOkResponse(VALID_PROFILE_ME));
    ...
    expect(screen.getByTestId('me-edit-saju-counter').textContent).toContain('?/2');
  ...
  it('AC1: submit → confirm → PATCH success → counter banner updates', async () => {
    ...
    render(<MeEditSajuPage fetchImpl={fetchImpl} />);
    await waitFor(() => expect(screen.getByTestId('me-edit-saju-loaded')).toBeInTheDocument());
    ...
    await waitFor(() => {
      expect(
        screen.getByText('수정 후엔 새 사주로 풀이가 나와요. 과거 히스토리는 그대로 남아요.'),
      ).toBeInTheDocument();
    });
    ...
    await waitFor(() => {
      expect(screen.getByTestId('me-edit-saju-counter').textContent).toContain('1/2');
    });
  ...
  it('AC2: backend 403 quota_exceeded → swap to 운영 문의 fallback', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('me-edit-saju-quota-exhausted')).toBeInTheDocument();
    });
  ...
  it('time-unknown checkbox disables the time input', async () => {
    ...
    const timeInput = screen.getByTestId('me-edit-saju-input-time') as HTMLInputElement;
