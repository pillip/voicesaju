...
/**
 * `/me/history/[id]` — Screen 19 (ISSUE-066): history player.
 *
 * Streams the archived audio for a past reading via the native
 * ``<audio>`` element, so the user can pause/seek/resume without
 * re-running the LLM pipeline. The route source is the
 * `GET /api/v1/reading/{id}/audio.mp3` endpoint that ISSUE-066 ships.
 *
 * Layout (top → bottom):
 *   1. TopAppBar with "풀이 다시 듣기" title + back affordance to /me.
 *   2. Archive ribbon "[YYYY-MM-DD] 풀이" (per ux_spec Screen 19).
 *   3. Native `<audio controls>` element bound to the audio URL.
 *      Pause/resume/seek are handled by the browser — no JS for AC3.
 *   4. Expired-audio fallback when the probe returns 410:
 *      "이 풀이는 더 이상 재생할 수 없습니다" + back-to-history link.
 *
 * State machine:
 *   - loading:  skeleton + aria-busy
 *   - error:    "잠시 후 다시 시도해주세요" + retry  (5xx / network / parse)
 *   - 401:      router.replace('/auth/login')
 *   - 404:      router.replace('/me')   (reading deleted / not owned)
 *   - 410:      expired-audio fallback (AC2)
 *   - loaded:   archive ribbon + `<audio>` element (AC1, AC3)
 *
 * AC mapping (ISSUE-066):
 *   AC1 → past reading → archived audio streams (test: probe → 200,
 *         audio element gets src set).
 *   AC2 → blob missing → "이 풀이는 더 이상 재생할 수 없습니다"
 *         (test: probe → 410, fallback rendered).
 *   AC3 → tap pause → audio stops (native browser behavior; the test
 *         asserts the `<audio controls>` element is in the tree).
 */
...
export interface MeHistoryItemPageProps {
  params: { id: string };
  /**
...
export default function MeHistoryItemPage({ params, fetchImpl }: MeHistoryItemPageProps) {
