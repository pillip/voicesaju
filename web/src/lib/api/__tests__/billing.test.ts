...
describe("fetchMySubscription", () => {
  it("returns the subscription envelope on 200 with row", async () => {
    const fetchImpl = vi
      .fn()
      .mockResolvedValueOnce(mkResponse({ subscription: SUB_ROW }));
  ...
  it("returns null subscription on 200 with empty envelope", async () => {
    const fetchImpl = vi
      .fn()
      .mockResolvedValueOnce(mkResponse({ subscription: null }));
  ...
  it("rejects an unexpected shape", async () => {
    const fetchImpl = vi
      .fn()
      .mockResolvedValueOnce(mkResponse({ wrong: "shape" }));

    await expect(fetchMySubscription(fetchImpl)).rejects.toBeInstanceOf(
      BillingFetchError,
    );
  });
});
...
describe("fetchPaymentHistory", () => {
  ...
  it("encodes the page query param", async () => {
    ...
    expect(fetchImpl).toHaveBeenCalledWith(
      "/api/v1/payments/history?page=3",
      expect.any(Object),
    );
  });
