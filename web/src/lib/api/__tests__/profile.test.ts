...
describe('fetchProfileMe (ISSUE-064)', () => {
  ...
  it('401 → ProfileFetchError with status=401', async () => {
    ...
    await expect(fetchProfileMe(fakeFetch as unknown as typeof fetch)).rejects.toMatchObject({
  ...
  it('malformed JSON → ProfileFetchError', async () => {
    ...
    await expect(fetchProfileMe(fakeFetch as unknown as typeof fetch)).rejects.toBeInstanceOf(
      ProfileFetchError,
    );
  });
...
function mkPatchErr(status: number, errorCode?: string): Response {
  return {
    ...
    status,
    json: async () =>
      errorCode ? { detail: { error: { code: errorCode, message: '' } } } : { detail: 'fail' },
  } as unknown as Response;
}
...
describe('patchProfile (ISSUE-071)', () => {
  it('200 → typed response with corrections_remaining', async () => {
    ...
    const out = await patchProfile(VALID_PATCH_BODY, fakeFetch as unknown as typeof fetch);
    expect(out.profile_id).toBe('p-1');
  ...
  it('PATCH includes the canonical URL + method + credentials', async () => {
    const fakeFetch = vi.fn(async (_url: RequestInfo | URL, _init?: RequestInit) =>
      mkOk(VALID_PATCH_RESPONSE),
    );
    await patchProfile(VALID_PATCH_BODY, fakeFetch as unknown as typeof fetch);
  ...
  it('403 with quota code → error.message === CORRECTION_QUOTA_EXCEEDED', async () => {
    const fakeFetch = vi.fn(async () => mkPatchErr(403, CORRECTION_QUOTA_EXCEEDED));
    try {
      await patchProfile(VALID_PATCH_BODY, fakeFetch as unknown as typeof fetch);
      ...
      expect((err as ProfileFetchError).message).toBe(CORRECTION_QUOTA_EXCEEDED);
    }
  });
  ...
  it('malformed body shape → ProfileFetchError', async () => {
    const fakeFetch = vi.fn(async () => mkOk({ profile_id: 'p-1', chart_id: 'c-2' }));
    await expect(
      patchProfile(VALID_PATCH_BODY, fakeFetch as unknown as typeof fetch),
